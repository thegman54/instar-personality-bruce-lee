"""
Personality Bruce Lee — Admin API Handlers
...
TABLE_PREFIX = "personality_bruce_lee"
...
async def list_traits(pool, slug, **_):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            FROM personality_bruce_lee_traits
    ...
    async with pool.acquire() as conn:
        try:
            row = await conn.fetchrow(
                """
                INSERT INTO personality_bruce_lee_traits
...
async def delete_trait(pool, slug, item_id=None, **_):
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM personality_bruce_lee_traits WHERE id = $1::uuid AND profile_slug = $2",
            item_id, slug,
        )
    if result == "DELETE 0":
...
async def list_quotes(pool, slug, **_):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            FROM personality_bruce_lee_quotes
            WHERE profile_slug = $1
...
async def create_quote(pool, slug, body=None, **_):
    ...
    async with pool.acquire() as conn:
        try:
            row = await conn.fetchrow(
                """
                INSERT INTO personality_bruce_lee_quotes
...
async def delete_quote(pool, slug, item_id=None, **_):
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM personality_bruce_lee_quotes WHERE id = $1::uuid AND profile_slug = $2",
            item_id, slug,
        )
    if result == "DELETE 0":
...
async def list_lexicon(pool, slug, **_):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            FROM personality_bruce_lee_lexicon
            WHERE profile_slug = $1
...
async def create_lexicon(pool, slug, body=None, **_):
    ...
    async with pool.acquire() as conn:
        try:
            row = await conn.fetchrow(
                """
                INSERT INTO personality_bruce_lee_lexicon
...
async def delete_lexicon(pool, slug, item_id=None, **_):
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM personality_bruce_lee_lexicon WHERE id = $1::uuid AND profile_slug = $2",
            item_id, slug,
        )
    if result == "DELETE 0":
...
async def list_reactions(pool, slug, **_):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            FROM personality_bruce_lee_reactions
            WHERE profile_slug = $1
...
async def create_reaction(pool, slug, body=None, **_):
    ...
    async with pool.acquire() as conn:
        try:
            row = await conn.fetchrow(
                """
                INSERT INTO personality_bruce_lee_reactions
...
async def delete_reaction(pool, slug, item_id=None, **_):
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM personality_bruce_lee_reactions WHERE id = $1::uuid AND profile_slug = $2",
            item_id, slug,
        )
    if result == "DELETE 0":
...
#
# Example: GET /skill_api/personality_bruce_lee/hal_test/traits
