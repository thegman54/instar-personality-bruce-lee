-- Personality Bruce Lee: Reactions table
-- Trigger → response pattern mappings for contextual behavior.

CREATE TABLE IF NOT EXISTS personality_bruce_lee_reactions (
...
CREATE INDEX IF NOT EXISTS idx_pbl_reactions_profile
    ON personality_bruce_lee_reactions(profile_slug);
CREATE INDEX IF NOT EXISTS idx_pbl_reactions_trigger
    ON personality_bruce_lee_reactions(profile_slug, trigger_type);
CREATE INDEX IF NOT EXISTS idx_pbl_reactions_intensity
    ON personality_bruce_lee_reactions(profile_slug, intensity);
CREATE INDEX IF NOT EXISTS idx_pbl_reactions_tags
    ON personality_bruce_lee_reactions USING GIN(tags);
