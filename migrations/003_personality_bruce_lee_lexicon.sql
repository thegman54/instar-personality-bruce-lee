-- Personality Bruce Lee: Lexicon table
-- Vocabulary fingerprint — words/phrases to favor, avoid, or use situationally.

CREATE TABLE IF NOT EXISTS personality_bruce_lee_lexicon (
...
CREATE INDEX IF NOT EXISTS idx_pbl_lexicon_profile
    ON personality_bruce_lee_lexicon(profile_slug);
CREATE INDEX IF NOT EXISTS idx_pbl_lexicon_type
    ON personality_bruce_lee_lexicon(profile_slug, usage_type);
CREATE INDEX IF NOT EXISTS idx_pbl_lexicon_freq
    ON personality_bruce_lee_lexicon(profile_slug, frequency);
